...
from mpc_rl_collision_avoidance.external.stable_baselines.common.callbacks import BaseCallback
from mpc_rl_collision_avoidance.external.stable_baselines.common.vec_env import VecEnv

if typing.TYPE_CHECKING:
    from mpc_rl_collision_avoidance.external.stable_baselines.common.base_class import BaseRLModel  # pytype: disable=pyi-error
...
class AbstractEnvRunner(ABC):
    def __init__(self, *, env: Union[gym.Env, VecEnv], model: 'BaseRLModel', n_steps: int):
        ...
        self.batch_ob_shape = (n_envs * n_steps,) + env.observation_space.spaces['observation'].shape
        #
        # self.obs = np.zeros((n_envs,) + env.observation_space.shape, dtype=env.observation_space.dtype.name)
        self.obs = np.zeros((n_envs,) + env.observation_space.spaces['observation'].shape, dtype="float32")
        #self.obs[:] = env.reset()
        self.obs[:] = env.reset()['observation']
