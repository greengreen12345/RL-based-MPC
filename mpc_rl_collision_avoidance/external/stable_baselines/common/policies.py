...
from mpc_rl_collision_avoidance.external.stable_baselines.common.tf_util import batch_to_seq, seq_to_batch
from mpc_rl_collision_avoidance.external.stable_baselines.common.tf_layers import conv, linear, conv_to_fc, lstm, dynamic_lstm
from mpc_rl_collision_avoidance.external.stable_baselines.common.distributions import make_proba_dist_type, CategoricalProbabilityDistribution, \
    MultiCategoricalProbabilityDistribution, DiagGaussianProbabilityDistribution, BernoulliProbabilityDistribution
from mpc_rl_collision_avoidance.external.stable_baselines.common.input import observation_input
...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch,
        ...
        with tf.variable_scope("input", reuse=False):
            ...
            state_ph_shape = (self.n_batch, ) + tuple(state_shape)
...
class LstmPolicy(RecurrentActorCriticPolicy):
        ...
        if net_arch is None:  # Legacy mode
            ...
            with tf.variable_scope("model", reuse=reuse):
                if feature_extraction == "cnn":
                    ...
                    # TODO: Make dimension variables generic
                    other_agents_features = extracted_features[:,6:]
                    ego_agent_features = extracted_features[:,:6]
                    other_agents_features = tf.reshape(other_agents_features,[n_batch,-1,9])
                    for i, layer_size in enumerate(layers):
                        extracted_features = act_fun(linear(extracted_features, 'pi_fc' + str(i), n_hidden=layer_size,
                                                            init_scale=np.sqrt(2)))
                input_sequence = batch_to_seq(extracted_features, self.n_env, n_steps)
                masks = batch_to_seq(self.dones_ph, self.n_env, n_steps)
                rnn_output, self.snew = dynamic_lstm(ego_agent_features,other_agents_features, self.dones_ph, self.states_ph, 'lstm1', n_hidden=n_lstm,seq_length=self.seq_length_ph,layer_norm=layer_norm)
    ...
    def step(self, obs, state=None, mask=None, deterministic=False,seq_length=None):
    ...
    def value(self, obs, state=None, mask=None, seq_length=None):
        if state is None:
            state = np.zeros([obs.shape[0],self.n_lstm])

        if seq_length is None:
            return self.sess.run(self.value_flat, {self.obs_ph: obs, self.states_ph: state, self.dones_ph: mask})
        else:
            return self.sess.run(self.value_flat, {self.obs_ph: obs, self.states_ph: state, self.dones_ph: mask, self.seq_length_ph: seq_length})
