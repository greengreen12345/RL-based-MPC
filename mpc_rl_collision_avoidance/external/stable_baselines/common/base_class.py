...
from mpc_rl_collision_avoidance.external.stable_baselines.common.misc_util import set_global_seeds
from mpc_rl_collision_avoidance.external.stable_baselines.common.save_util import data_to_json, json_to_data, params_to_bytes, bytes_to_params
from mpc_rl_collision_avoidance.external.stable_baselines.common.policies import get_policy_from_name, ActorCriticPolicy
from mpc_rl_collision_avoidance.external.stable_baselines.common.runners import AbstractEnvRunner
from mpc_rl_collision_avoidance.external.stable_baselines.common.vec_env import (VecEnvWrapper, VecEnv, DummyVecEnv,VecNormalize, unwrap_vec_normalize)
from mpc_rl_collision_avoidance.external.stable_baselines.common.callbacks import BaseCallback, CallbackList, ConvertCallback
...
class ActorCriticRLModel(BaseRLModel):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=False,seq_length=None):
        if state is None:
        ...
        #print("vectorized_env", vectorized_env)

        observation = observation.reshape((-1,) + self.observation_space.shape)
        if seq_length is None:
            actions, _, states, _ = self.step(observation, state, mask, deterministic=deterministic)
        else:
            actions, _, states, _ = self.step(observation, state, mask, deterministic=deterministic,seq_length=seq_length)
